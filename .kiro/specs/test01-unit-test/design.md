...
This design specifies a JUnit 5 unit test suite for the Todo Management Spring Boot backend. The suite validates business logic (services) and security components (PasswordValidator, JwtUtil, AuthInterceptor) using isolated mocks. No HTTP layer is tested here — all endpoint behavior is covered by test02-api-integration-test.

The test architecture has two tiers:
...
- Test count target: ~50 example-based tests + 3 property tests (100+ tries each)
- All tests are deterministic (no shared state, no real I/O, no HTTP)
...
### Infrastructure Conventions (Requirement 8)

1. Service/Security tests: `@ExtendWith(MockitoExtension.class)` — no Spring context
2. Package mirroring: test classes in `.service` and `.security` sub-packages
3. Naming: `methodUnderTest_stateOrInput_expectedBehavior`
4. Grouping: `@Nested` inner classes per logical context
5. Readability: `@DisplayName` on classes and nested groups
6. Strictness: STRICT_STUBS (MockitoExtension default) — no unused stubbing
7. Data-driven: `@ParameterizedTest` + `@MethodSource` for PasswordValidator boundary tests
