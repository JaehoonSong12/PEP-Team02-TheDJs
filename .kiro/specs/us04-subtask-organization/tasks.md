...
a `SubtaskNotFoundException` handler in `GlobalExceptionHandler`, and the full test suite
(service and controller layers). The service layer (`SubtaskService`) and repositories are
already implemented. Tasks are ordered so each step builds on the previous ones.

---

## Tasks

- [ ] 1. Add `SubtaskNotFoundException` handler to `GlobalExceptionHandler`
  - Open `exception/GlobalExceptionHandler.java` (create it if it does not exist yet)
  - Add `@ExceptionHandler(SubtaskNotFoundException.class)` method
  - Return HTTP 404, body `{"status": 404, "message": "..."}`
  - Confirm handlers for `TaskNotFoundException` (404), `TaskOwnershipException` (403), and `IllegalArgumentException` (400) are also present
  - _Requirements: 7.1, 7.2, 7.3, 7.4, 7.5_

- [ ] 2. Create `SubtaskController` in the `controller/` package
  - Create `controller/SubtaskController.java`
  - Annotate with `@RestController`, `@RequestMapping("/api/todos/{id}/subtasks")`, `@RequiredArgsConstructor`
  - Inject `SubtaskService` via a final field
  - [ ] 2.1 Implement `GET /api/todos/{id}/subtasks`
    ...
    - _Requirements: 1.1_
  - [ ] 2.2 Implement `POST /api/todos/{id}/subtasks`
    ...
    - _Requirements: 2.1_
  - [ ] 2.3 Implement `GET /api/todos/{id}/subtasks/{subtaskId}`
    ...
    - _Requirements: 3.1_
  - [ ] 2.4 Implement `PUT /api/todos/{id}/subtasks/{subtaskId}`
    ...
    - _Requirements: 4.1_
  - [ ] 2.5 Implement `DELETE /api/todos/{id}/subtasks/{subtaskId}`
    - Accept `@RequestAttribute UUID userId`, `@PathVariable UUID id`, `@PathVariable UUID subtaskId`
...
- [ ] 3. Checkpoint — compile check
  ...
  - Ensure all imports in `SubtaskController` and `GlobalExceptionHandler` resolve

- [ ] 4. Write `SubtaskServiceTest`
  ...
  - [ ] 4.1 Create subtask — valid title
...
- [ ] 5. Write `SubtaskControllerTest`
  ...
  - [ ] 5.2 POST /api/todos/{id}/subtasks — valid body → HTTP 200
    ...
    - _Requirements: 2.1_
  - [ ] 5.3 POST /api/todos/{id}/subtasks — blank title → HTTP 400
  ...
  - [ ] 5.4 GET /api/todos/{id}/subtasks/{subtaskId} — not found → HTTP 404
  ...
  - [ ] 5.5 GET /api/todos/{id}/subtasks — unknown parent task → HTTP 404
    - Mock `subtaskService.getSubtasksForTask` to throw `TaskNotFoundException`
    - Assert status 404
    - _Requirements: 1.4_
  - [ ] 5.6 GET /api/todos/{id}/subtasks — wrong owner → HTTP 403
    ...
    - _Requirements: 1.5_
  - [ ] 5.7 PUT /api/todos/{id}/subtasks/{subtaskId} — valid body → HTTP 200
    - Mock `subtaskService.updateSubtask` to return the updated subtask
    - Assert status 200
    - _Requirements: 4.1_
  - [ ] 5.8 DELETE /api/todos/{id}/subtasks/{subtaskId} → HTTP 204
